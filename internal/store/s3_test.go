...
func TestExtractServiceFromBucketURL(t *testing.T) {
	tests := []struct {
		name        string
		url         string
		want        string
		...
		{
			name:    "s3 scheme with path",
			url:     "s3://my-bucket/prefix/path",
			want:    "s3",
			wantErr: false,
		},
		{
		...
		},
		{
			...
			want:        "",
			wantErr:     true,
			errContains: "failed to parse bucket URL",
		},
		{
			name:        "missing scheme",
			url:         "my-bucket",
			want:        "",
			wantErr:     true,
			errContains: "bucket URL must have a scheme",
		},
		{
			...
			want:        "",
			wantErr:     true,
			errContains: "unsupported URL scheme \"http\"",
		},
		{
			...
			want:        "",
			wantErr:     true,
			errContains: "unsupported URL scheme \"https\"",
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := extractServiceFromBucketURL(tt.url)
